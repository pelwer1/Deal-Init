...
// 1.7 added --quiet to minimize chat messages to the gm
// 1.8 fixed controlledby bug in PC/NPC token detection;
//     fixed bug in --quiet where it would toggle on/off
//     added --showComps to detect and flag clubs in initiative for dramatic task and chase complications
...
x    --help
x    --reset with/without --quiet
x    --4jreset with/without --quiet
x    (no args)
x    --clearTurnOrder
x    --show
x    --onlyto --string
...
x  Qui = Quick
x  LH  = Level Headed
x  ILH = Improved Level Headed
x  TT  = Tactician (cards dealt to chat)
x  MTT = Master Tactician (cards dealt to chat)
x  HH  = Hesitant Hindrance
x  WCE = Any Joker Activated Wild Card Edge (announced in turn order)

x- On Hold
x- Joker detection
x- Sorting
x- Token not associated with characters
x- Custom items (non token entry in turn order)
...
var DealInit = DealInit || (function() {
  ...
  var version = '1.8',
    lastUpdate = '[Last Update: Apr 10, 2022, 5pm Pacific]',
    jokerLastRound = 0,
    jokerInChat = 0,
    fourJokers = 0,
    onlyToString = '',
    ...
    getInitiativeEdges = function(id) {
      ...
      for (i = 0; i < turnorder.length; i += 1) {
        ...
        // turn order item is a "token that represents a character", look for init edges
          ...
          // For tokens that are linked to Characters, the controlledby field on the token
          // is overridden by the controlledby field on the Character.

          // from the token graphic id, get the token object
          token_obj = getObj("graphic", toid);
          
          // determine who controls the token - get the character the token represents
          let characterObj = getObj('character', token_obj.get('represents'))

          // default the controler to blank (npc)
          let controler = ''
         
          // the the token represents the charaters get the list of player ids that controll it
          if (characterObj) {
             controler = characterObj.get('controlledby')
          }
        
          if (verboseMode) { sendChat('','controlled by: ['+controler+']'); }

          // if player is not the GM, mark as a 'pc'
          if (controler === '' || playerIsGM(controler)) {
            tokentype = 'npc';  // this check failing ot detect pcs
          } else {
            tokentype = 'pc';
          }
    ...
    doTheDeal = function(id) {
          ...
          if ((turnorder[i].rank > 51 && turnorder[i].rank < 60) || jokerInChat) {
            ...
            if (initEdges[i].edges.indexOf('WCE') !== -1) {
              ...
              sendChat('', sendto + divStart + '<div style="font-weight: bold; border-bottom: 1px solid black;font-size: 130%;">' +
                initEdges[i].shortname + '</div>Your Joker activates your <b>Wild Card edge!</b>' + divEnd);
            }
            jokerInChat = 0;
            if (verboseMode) {
              var whoVM = getObj('player', id).get('_displayname').split(' ')[0];
      ...
      // clear dealing options: onlyto match string, deal2chat, showComps
      onlyToString = '';
      dealToChat   = 0;
      showComps    = 0;
    ...
    showHelp = function(id) {
      ...
      sendChat('',
        ...
        '<b><span style="font-family: serif;">!deal-init ' + '[ <i>--help</i> ] [<i>--[4j]reset [--quiet]</i> ] [<i>--show</i> ] [<i>--onlyto --string</i> ] [<i>--deal2chat</i> ]' + '</span></b>' +
        ...
        '</li> ' +
        '<li style="border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;">' +
        '<b><span style="font-family: serif;">' + '--reset' + '</span></b> ' +
        ' Reset the deck and shuffle.  Use at the start of a new scene or encounter.  Add '+
        '<b><span style="font-family: serif;">' + '--quiet' + '</span></b>  to minimize chat messages.' +
        '</li> ' +
        '<li style="border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;">' +
        '<b><span style="font-family: serif;">' + '--4jreset' + '</span></b> ' +
        ' Reset the deck with 4 Jokers and shuffle.  Use at the start of a new scene or encounter.  Add '+
        '<b><span style="font-family: serif;">' + '--quiet' + '</span></b>  to minimize chat messages.' +
        '</li> ' +
        '<li style="border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;">' +
        ...
        '</li> ' +
        '<li style="border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;">' +
        '<b><span style="font-family: serif;">' + '--showComps' + '</span></b> ' + ' to flag complications (clubs) in chat' +
        '</li> ' +
        '</ul>' +
        '</div>' +
        '</div>' +
    ...
    //     --quiet as 2nd arg when reseting/shuffling the deck to minimize chat messages
    // --show to show the cards in turnorder, discard, draw piles (showCards)
    // --onlyto --string- deals cards only to names that contain string (case sensitive)
    // --showComps to show complications in chat on clubs
    ...
    handleInput = function(msg_orig) {
      ...
      // show complicaitons in chat
      if (args[0] === "showComps") {
        showComps = 1;
        // do not return - need to flow thru to normal dealing process;
        // log('-=> DealInit: Using [showComps] option. <=- ');
      }
